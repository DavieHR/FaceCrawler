#使用方法：在命令行输入python xtq_main_video.py -i ./test_kf/ -o ./test_save -t ./00424.png -s 80 -v ./movie1.mp4
#i是存放关键帧图片的文件夹，记得加‘/’;o是输出处理后的图片的文件夹；t是模板照片;s是脸部质量的分数，默认75；v是视频路径
import argparse
import cv2
import numpy as np
import pprint
...
def get_args():
    ...
    parser.add_argument("-v", "--video", type=str, default="./movie1.mp4", help="The path of video.")
...
if __name__ == "__main__":
    ...
    #读取视频并处理
    Get_Imgvec(soc,dst)     #=============输入源文件和目标文件,这步会提取关键帧

    #获取模板特征
    template_landmarks,nu = get_landmarks(TEMPLATE_PATH,0)
